...
import ru.yandex.practicum.service.similarity.SimilarityService;
...
public class SimilarityProcessor implements Runnable {
    ...
    private final static String SIMILARITY_PROCESSOR_CONSUMER = "similarity-processor";
    ...
    public void run() {
        try {
            consumer = kafkaClient.getConsumer(SIMILARITY_PROCESSOR_CONSUMER, ConsumerPropertiesStorage.getSimilarityProcessorConsumerProperties());
            consumer.subscribe(List.of(topicNames.getSimilarity()));
            while (true) {
                ConsumerRecords<Long, SpecificRecordBase> records = consumer.poll(POLL_DURATION_TIMEOUT);
                int count = 0;
                for (ConsumerRecord<Long, SpecificRecordBase> record : records) {
                    EventSimilarityAvro userActionAvro = (EventSimilarityAvro) record.value();
                    similarityService.addSimilarity(userActionAvro);
            ...
            try {
                ...
                kafkaClient.stopConsumer(SIMILARITY_PROCESSOR_CONSUMER);
            }
        }
    }
}
